...
(function () {
  ...
  const CFG = {
    ...
    deal: {
      color: '#a855f7',
      brand: 'WizeDeal',
      // A brand-new user has zero deals — Compare/Saved/Share/Portfolio are all
      // dead-ends for them. Lead with the core entry (analyze a new deal), then
      // a sample deal to explore. Both live on the homepage ('/').
      steps: [
        { icon: '🏠', href: '/', k: 'analyze', primary: true },
        { icon: '✨', href: '/', k: 'sample' },
      ],
    },
  };
  ...
  const TR = {
    he: {
      ...
      deal:    { analyze: 'נתח עסקה חדשה', sample: 'נסה עסקה לדוגמה' },
    },
    en: {
      ...
      deal:    { analyze: 'Analyze a new deal', sample: 'Try a sample deal' },
    },
    pt: {
      ...
      deal:    { analyze: 'Analisar nova oferta', sample: 'Testar oferta de exemplo' },
    },
    es: {
      ...
      deal:    { analyze: 'Analizar nueva oferta', sample: 'Probar oferta de ejemplo' },
    },
  };
  ...
  function build(app, lang) {
    ...
    const stepsHtml = cfg.steps
      ...
      .map(function (s) {
        // The primary action (the core entry) is visually emphasised with the
        // brand colour fill so a zero-deal user knows where to start.
        var bg = s.primary
          ? cfg.color + '26'
          : 'rgba(255,255,255,0.04)';
        var border = s.primary
          ? '1px solid ' + cfg.color + '88'
          : '1px solid rgba(255,255,255,0.08)';
        var weight = s.primary ? '700' : '600';
        return '<a href="' + s.href + '" class="wlqs-step" data-k="' + s.k + '" style="'
          ...
          + '<span style="font-size:14px;font-weight:' + weight + ';">' + tr[app][s.k] + '</span>'
          + '</a>';
      })
      .join('');
